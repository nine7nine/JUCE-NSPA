...
// All winelib host plumbing is now in two shared headers next to this
// one — extracted so juce_VSTPluginFormatImpl.h (VST2 host) can use
// the same WineWin32Dispatcher type + Wine API surface + path helpers
// instead of duplicating them.  Order matters: pi_sync depends on the
// vendored librtpi header it includes; dispatch depends on pi_sync.
...
private:
    ...
    bool open()
    {
        ...
        if (winelib_is_pe_file (dllFile))
        {
            ...
            globalWineDispatcher().run ([&] { peHandle = LoadLibraryW (widePath.getData()); });
