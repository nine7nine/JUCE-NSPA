...
class OpenGLContext::WaylandNativeContext : public OpenGLContext::NativeContext
{
    ...
    void triggerRepaint()
    {
        // NSPA: matches upstream X11 NativeContext::triggerRepaint -- the plugdata
        // wayland branch shipped this commented out, which silently dropped GL
        // repaint requests (Element's main window relies on them).
        if (context != nullptr)
