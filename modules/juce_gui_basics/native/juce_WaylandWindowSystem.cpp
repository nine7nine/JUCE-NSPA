...
struct WaylandWindow {
    ...
    bool initialConfigureDone:1 = false; // NSPA: was a file-static; per-window avoids races between concurrent createWindow calls
};
...
WaylandWindow* WaylandWindowSystem::createWindow (bool isSubsurface, ComponentPeer* peer, bool alwaysOnTop, WaylandWindow* parent)
{
    ...
    else
    {
        ...
        // NSPA: null-check BEFORE SetCapabilities -- plugdata branch called
        // decorFrameSetCapabilities on a possibly-null frame, which would
        // segfault on libdecor failure (e.g. compositor without xdg_decoration).
        if (! window->handle.frame)
        {
            jassertfalse; // Critical error, could not create window frame
            // window isn't in surfaceToWindow yet (that happens after the if/else block);
            // only the surface needs explicit destroy here.
        ...
        // Wait for initial configure (NSPA: per-window flag, not file-static)
        int timeout = 50;
        while (! window->initialConfigureDone && timeout-- > 0)
        {
            WaylandSymbols::getInstance()->displayDispatchPending (display);
            if (! window->initialConfigureDone)
            {
                usleep (10000);
...
void WaylandWindowSystem::setBounds (WaylandWindow* window, Rectangle<int> b)
{
    ...
    else
    {
        ...
        // NSPA: plugdata branch leaked the libdecor_state per resize; the
        // configure callback frees its own state but this code path didn't.
        WaylandSymbols::getInstance()->decorStateFree (state);
    }
}
