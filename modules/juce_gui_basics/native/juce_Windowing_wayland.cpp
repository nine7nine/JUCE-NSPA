...
class WaylandComponentPeer final : public ComponentPeer, public Timer
{
    ...
    {
        // NSPA: plugdata wayland branch returned true unconditionally, which
        // forced JUCE's repaint-skip logic to keep painting minimised windows.
        // Return the actual minimised state via the WaylandWindowSystem.
    ...
    {
        // NSPA: Wayland uses the .desktop file's Icon= entry; there is no
        // per-surface icon protocol (xdg-toplevel-icon-v1 is unstable and
        // not widely deployed).  Quiet no-op -- Element MainWindow calls
        // setIcon at startup and the original jassertfalse fired every time.
